...
### 專案目標
開發一個 RDP 鍵盤轉換器，解決 Android RDP 軟鍵盤與高性能應用程式（如 Warp Terminal、VS Code Claude 擴展）之間的兼容性問題。
...
### 核心問題
- **Warp Terminal** 僅監聽低級硬體掃描碼事件以優化性能
- **Android RDP 軟鍵盤** 發送高級軟體事件（VK_PACKET）到 Windows
- **結果**：Warp Terminal 無法檢測到 RDP 軟鍵盤輸入
...
### 1. VK_PACKET 結構解析
**發現**：RDP 軟鍵盤發送的 VK_PACKET 事件中，Unicode 字符存儲在 `scanCode` 欄位而不是 `vkCode` 欄位。
...
### 2. RDP 安全限制繞過
**發現**：系統級 `SendInput` 和 `keybd_event` 在 RDP 環境中被限制，但直接向視窗發送消息不受限制。
...
### 短期優化 (1-2 週)
1. **性能/策略優化**
   - 移除對剪貼簿的依賴（優先目標）；在未完全移除前，暫時優化中文輸入的剪貼簿操作延遲
   - 優化字符檢測邏輯，減少不必要的處理
...
2. **RdpKeyboardTranslator.csproj** - 項目配置
3. **app.manifest** - 管理員權限配置

### 建置腳本
1. **build.bat** - 編譯腳本
2. **run_admin.bat** - 管理員權限運行
3. **run_debug.bat** - 調試模式運行
...
本專案成功解決了 Android RDP 軟鍵盤與高性能應用程式的兼容性問題，實現了：
